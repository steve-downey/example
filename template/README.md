...
The code is trivial so that I can repurpose the framework quickly. A library that returns my name, a test that confirms that works, and an example hello `[[ library_name ]]` that uses the library.
...
```shell
([[ project_slug ]]) sdowney@pwyll:~/src/surround/[[ project_slug ]] (main ±)
$ make help
